    ...
    return (
        <View className="bg-white/15 rounded-2xl p-6">
            ...
            {forecast.map((day: any, i: number) => (
                ...
                >
                    <Text className="font-bold w-1/3 text-white">{dayjs(day.date).format('ddd') ?? '..'}</Text>
                    ...
                    <View className="w-1/3 flex-row justify-end space-x-4">
                        <Text className="font-medium text-white">H: {Math.round(day?.day?.maxtemp_c) || '..'}°</Text>
                        <Text className="font-medium text-white/70">L: {Math.round(day?.day?.mintemp_c) || '..'}°</Text>
                    </View>
                </View>
            ))}
        </View>
