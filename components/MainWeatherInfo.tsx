...
export default function MainWeatherInfo({ weather, location }: { weather: any, location: any }) {
    return (
        <View className="items-center mb-8 my-8">
            ...
            <Text className="text-lg text-white/80 mb-4">{dayjs(location?.localtime).format('dddd, D MMMM') ?? '..'}</Text>
            <View className="my-4">
                <Image
            ...
            <Text className="text-5xl font-thin text-white">{Math.round(weather?.temp_c) || '..'}°</Text>
            <Text className="text-lg text-white/90 capitalize mb-1 text-center">{weather?.condition.text || '..'}</Text>
            <Text className="text-lg text-white">Feels Like {Math.round(weather?.feelslike_c) || '..'}°</Text>
        </View>
    )
}
